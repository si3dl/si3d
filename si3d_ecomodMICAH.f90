...
! ... Arguments
  ...
  !. . . Local Variables
  REAL    ::   Tk, lnOS, OS, ln_Pwv, Pwv, theta2, Patm,f_SOD,k_a2,Wx_mag
   

!  ! Calculate DO saturation
  Tk = salp(kwq,lwq) + 273
  lnos = -139.34411 + 1.575701*1E5 /(Tk    ) &
  &                 - 6.642308*1E7 /(Tk**2.) &
  &                 + 1.243800*1E10/(Tk**3.) &
  &                 - 8.621949*1E11/(Tk**4.)
  OS = EXP(lnos)
...
sourcesink(kwq,lwq,LDO) = sourcesink(kwq,lwq,LDO)
 ...
 IF (kwq .eq. k1z(lwq)) THEN
      ...
      Wx_mag  = SQRT (uair(lwq)**2.+vair(lwq)**2.);
	  
  ! Calculate wind magnitude from vector components
   !Wx_mag = (uair2d**2 + vair2d**2)**(1/2)


  ! Calculate wind speed dependent reareation coeff (Banks 1975)
   k_a2 = ((0.728*Wx_mag**(1/2)) - (0.317*Wx_mag) + (0.0372*Wx_mag**2))/hpp(kwq,lwq)

   !k_a2 = k_a2*idt ! You are already doing ka2 calculations for each time step. I do not see this necessary

 sourcesink(kwq,lwq,LDO) = sourcesink(kwq,lwq,LDO)       &
         &       +    k_a2*(OS - tracerpp(kwq,lwq,LDO))   !reaeration
