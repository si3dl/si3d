...
!-----------------------------------------------------------------------
  ...
  REAL             :: densty_s, rhoguess,rhomin,rhomax,delta, densw,   &
    ...
    ! maxiter = 20
    ! DO WHILE (delta > 1e-4)
    !   rhoguess = (rhomin+rhomax)/2
    !   pressureh = rhoguess*9.806*elevation
    !   pressure = 1e-5*pressureh
    !
    !   pressuremin = rhomin*9.806*elevation
    !   pressuremin = 1e-5*pressuremin
    !
    !   densw = 999.842594                    &
    !         + 6.793952e-2*temperature       &
    !         - 9.095290e-3*temperature**2    &
    ...
    !       - 5.3009e-4*temperature**2)
    !
    !   kmin = kw + salinity*(54.6746 - 0.603459*temperature             &
    !       + 1.09987e-2*temperature**2                               &
    !       - 6.1670e-5*temperature**3)                               &
    !       + salinity**(3/2)*(7.944e-2                               &
    !       + 1.6483e-2*temperature                                   &
    ...
    !       + 2.0816e-8*temperature + 9.1697e-10*temperature**2)
    !
    !   kmin = kmin + pressuremin*(3.239908                           &
    !       + 1.43713e-3*temperature + 1.16092e-4*temperature**2      &
    !       - 5.77905e-7*temperature**3)                              &
    !       + pressuremin*salinity*(2.2838e-3                         &
    !       - 1.0981e-5*temperature                                   &
    !       - 1.6078e-6*temperature**2)                               &
    !       + 1.91075e-4*pressuremin*salinity**(3/2)                  &
    !       + pressuremin**2*(8.50935e-5                              &
    !       - 6.12293e-6*temperature                                  &
    !       + 5.2787e-8*temperature**2)                               &
    !       + pressuremin**2*salinity*(-9.9348e-7                     &
    !       + 2.0816e-8*temperature + 9.1697e-10*temperature**2)
    !
    !   of = rhoguess - densws/(1-pressure/k)
    !   ofmin = rhomin - densws/(1-pressuremin/kmin)
    !   IF (of*ofmin > 0) THEN
    !     rhomin = rhoguess
    !   ELSEIF (of*ofmin <0) THEN
    !     rhomax = rhoguess
    !   ENDIF
    !   delta = abs(of)
    !   iter = iter + 1
