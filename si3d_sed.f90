...
! --------------------------------------------------------------------
  ...
  do i = 1, sedNumber
    ...
    ! if (lwq == 5) then
    ! print*,'i = ',i,'h = ',h(kwq + 1, lwq),'dx = ',dx,'dy = ',dy
...
! --------------------------------------------------------------------
  ...
  call tauCritical(tauCrt, g, sed_diamm, submerged_spec_g, w_dens, kinematic_viscosity, Rep)
...
! --------------------------------------------------------------------
  ...
  tauCrt = shields_param * g * submerged_spec_g * sed_diamm * w_dens
